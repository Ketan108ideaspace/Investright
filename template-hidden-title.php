<?php 
/* Template Name: Hidden Title With No sidebar Template */
get_header();
?>
<!-- Body Content -->
						...
						while ( have_rows('widget_layout') ) : the_row();
							...
							if($widget_type=="singlewidget") {
								...
								if($widget_content_type=="subscribe") {
									...
									if($widget_title!="") {
										//echo '<div class="widget-poll"><h4>'.$widget_title.'</h4><div class="widget-poll-inner">'.$widget_description.'</div></div>';
										echo '<div class="widget-poll"><div class="widget-poll-inner">'.$widget_description.'</div></div>';
									}
								} else {}
							} else {
								$widget_content_type = get_sub_field('widget_content_type');
